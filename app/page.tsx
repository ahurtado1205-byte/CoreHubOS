...
export default function Home() {
  ...
  const displayItems = useMemo(() => {
    ...
    const convertedQuoteIds = new Set(bookings.map(b => b.quote_id).filter(Boolean));
    ...
    const virtualQuotesFromBookings = bookings.filter(b => {
      if (!startDate && !endDate) return true;
    ...
    return allItems.filter(item => 
      ...
      (item.confirmation_id && item.confirmation_id.toLowerCase().includes(q)) ||
      item.id.toLowerCase().includes(q)
    );
  }, [quotes, bookings, searchQuery, startDate, endDate]);

  // KPI Calculations
  const openQuotes = displayItems.filter(q => !['booked', 'lost', 'expired'].includes(q.status));
  const quotesValue = openQuotes.reduce((acc, q) => acc + (q.total_amount || 0), 0);
  const pendingTasks = tasks.filter(t => t.status === 'pending');
  
  const bookedQuotes = displayItems.filter(q => q.status === 'booked');
  const bookedValue = bookedQuotes.reduce((acc, q) => acc + (q.total_amount || 0), 0);
  
  const today = new Date();
  const inactiveQuotes = openQuotes.filter(q => {
  ...
  const handleOpenQuote = (item?: any) => {
    if (item?.is_booking) {
      const realBooking = bookings.find(b => b.id === item.id);
      if (realBooking) {
                      ...
                      >
                        ...
                        {currentPropertyId === 'all' && (
                          <div className="text-[10px] text-indigo-600 font-bold mb-1 truncate">
                            {properties.find(p => p.id === quote.property_id)?.name}
                          </div>
                        )}
            ...
            <div className="grid grid-cols-2 gap-4 mt-6">
              <button 
                onClick={() => {
                  ...
                  const tpl = templates.find(t => t.id === templateIdToUse);
                  if (tpl) {
                    msg = tpl.content
